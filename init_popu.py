...
def getperpen(line):
    ...
    # print(paral)
    perpen = LineString([paral1.boundary.geoms[1], paral2.boundary.geoms[0]])
    return perpen
...
def plotline(line):
    point1 = line.boundary.geoms[0]
    point2 = line.boundary.geoms[1]
...
def convertP2P(point, pointx, start, end):
    a = start[1] - end[1]
    b = end[0] - start[0]
    c = -(a * start[0] + b * start[1])

    pointy = np.abs(a * point.x + b * point.y + c) / np.sqrt(a*a + b*b) 
...
if __name__ == "__main__":
    ...
    for i in range(numR):
        if iniPopulationCoord[i]:
            pointy = convertP2P(iniPopulationCoord[i], (i+1)*R, start, end)
