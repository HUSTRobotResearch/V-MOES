...
public class PSO {
	...
	public int[] particleRank(Path[] particles, int type) {
		...
		for (int i = 0; i != len; i++) {
			...
			for (int j = 0; j != len; j++) {
				if (j != i && obj[j] >= obj[i]) {
					count++; // Count number of particles that is dominated particle i
				}
	...
	// Return index of particle from highest rank to lowest rank
	...
	public double[] crowdingDistance(Path[] particles) {
		...
		int index = 0; // Count number of particles that is null
		for (int i = 0; i != len; i++) {
			if (particles[i].points[0] == null) {
				index++;
			}
	...
	public void addArchive(Path par, Path[] NaParticles) {
		...
		if (checkNull == true) { // if Na null, add to archive
			...
			// If par is not dominated by any Na particle, add to archive
			if (!dominated) {
				if (dominate) {
					for (int j = 0; j != numR; j++) {
					...
					if (breakPoint != 100) { // Na not full, add directly
						for (int j = 0; j != numR; j++) {
						...
						// Replace Na particle with worst CD
						if (worstCDid != Nmax) {
							for (int i = 0; i != numR; i++) {
